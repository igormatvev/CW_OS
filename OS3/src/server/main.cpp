...
void sendToAll(char msg[BUFFER_SIZE], client* newClient) {
    mutex.lock();
    char buffer[BUFFER_SIZE];
    memset(buffer, 0, sizeof(buffer));
    buffer[0] = '\0';
    strcpy(buffer, msg);
    char* ptr = strstr(buffer, newClient->nickname);
    if (ptr != NULL) {
        int l = strlen(newClient->nickname);
        for (int i = 0; i < l; i++) {
            buffer[i] = ' ';
        }
        buffer[0] = 'Y';
    ...
    for(unsigned int i = 0; i < clients.size(); i++) {
        if(strcmp(clients[i].nickname, newClient->nickname) == 0) {
            if(send(clients[i].socketHandle, buffer, BUFFER_SIZE, 0) < 0)
